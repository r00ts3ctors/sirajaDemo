<?php
    ...
    if(empty($_SESSION['admin'])){
        ...
        if(isset($_REQUEST['submit'])){
                ...
                if($_REQUEST['kode'] == "" || $_REQUEST['nama'] == "" || $_REQUEST['kota'] == "" || $_REQUEST['uraian'] == ""){
                ...
                if(!preg_match("/^[a-zA-Z0-9. ]*$/", $kode)){
                    ...
                    if(!preg_match("/^[a-zA-Z0-9.,\/ -]*$/", $nama)){
                        ...
                        if(!preg_match("/^[a-zA-Z0-9.,()\/\r\n -]*$/", $uraian)){
                            ...
                            $query = mysqli_query($config, "UPDATE tbl_klasifikasi SET kode='$kode', nama='$nama', kota='$kota', uraian='$uraian', id_user='$id_user' WHERE id_klasifikasi='$id_klasifikasi'");

                            if($query != false){
                                $_SESSION['succEdit'] = 'SUKSES! Data Kategoti Klinik berhasil diupdate';
                        ...
                        <div class="col s12">
                            <nav class="secondary-nav">
                                <div class="nav-wrapper blue-grey darken-1">
                                    <ul class="left">
                                        <li class="waves-effect waves-light"><a href="#" class="judul"><i class="material-icons">edit</i> Edit Kategori Klinik</a></li>
                                    </ul>
                                </div>
                            </nav>
                        </div>
                        ...
                        <form class="col s12" method="post" action="?page=ref&act=edit">
                            ...
                            <div class="row">
                                ...
                                <i class="material-icons prefix md-prefix">place</i>
                                    <select class="validate" name="kota" id="kota" required>
                                        <option value="">Pilih </option>
                                        <?php 
                                          $sql=mysqli_query($config,"SELECT * FROM tbl_kota");
                                          while ($datax=mysqli_fetch_array($sql)) { 
                                            if($datax['id'] ==  $row['kota']){
                                                $selected = "selected";
                                            } else {
                                                $selected = "";
                                            }
                                        ?>
                                           <option value="<?php echo $datax['id']?>" <?php echo $selected; ?>><?php echo $datax['nama_kota']?></option> 
                                         <?php
                                          }
                                         ?>
                                    </select>
