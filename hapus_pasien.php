    	...
    	if(mysqli_num_rows($query) > 0){
            ...
            <!-- Row form END -->';
            	...
            	if(isset($_REQUEST['submit'])){
                    ...
                    $find = mysqli_query($config, "SELECT * FROM tbl_surat_masuk WHERE id_surat='$id_surat'");
                    if(mysqli_num_rows($find) > 0){
                        while($dta = mysqli_fetch_array($find)){
                             $query = mysqli_query($config, "DELETE FROM tbl_user WHERE username='".$dta['no_surat']."'");
                            //print_r($row); die();
                        }
                    }
                    // die();
                    ...
                    if(!empty($row['file'])){
                        ...
                        $query3 = mysqli_query($config, "DELETE FROM tbl_user WHERE username='$id_surat'");
